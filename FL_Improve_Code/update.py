...
class LocalUpdate(object): # 定义客户端本地更新过程的类
    ...
    def update_weights(self, model, global_round): # 执行本地模型更新 (训练)
        ...
        # Set learning rate scheduler (根据参数选择学习率调度器)
        ...
        for iter_epoch in range(self.args.local_ep): # 进行指定次数的本地 epoch 训练
            ...
            for batch_idx, (images, labels) in enumerate(self.trainloader): # 遍历本地训练数据加载器
                ...
                # 更新学习率 (每个batch后)
                if scheduler is not None and self.args.lr_scheduler != 'none':
                    scheduler.step()
    ...
    def _get_scheduler(self, optimizer):
        ...
        根据参数选择并创建学习率调度器
        """
        if self.args.lr_scheduler.lower() == 'none':
            return None
            ...
            raise ValueError(f"不支持的学习率调度器: {self.args.lr_scheduler}. 支持的调度器: ['none', 'step', 'exp', 'cosine']")
