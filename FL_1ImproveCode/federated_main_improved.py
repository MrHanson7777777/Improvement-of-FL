...
if __name__ == '__main__':
    ...
    print(start_time)
    print('改进版本（深度CNN架构 - ResNet/EfficientNet）')
    ...
    if args.model == 'cnn':
        ...
        # Enhanced CNN models for better performance
        if args.dataset == 'cifar':
            global_model = EnhancedEfficientNetCifar(num_classes=10)
        elif args.dataset == 'cifar100':
            global_model = EnhancedEfficientNetCifar(num_classes=100)
        else:
            print(f"Enhanced CNN not available for {args.dataset}, using regular CNN")
            if args.dataset == 'mnist':
                global_model = CNNMnist(args=args)
            elif args.dataset == 'fmnist':
                global_model = CNNFashion_Mnist(args=args)
        ...
        # ResNet-50 for CIFAR
        if args.dataset == 'cifar':
            global_model = ResNet50Cifar(num_classes=10)
        elif args.dataset == 'cifar100':
            global_model = ResNet50Cifar(num_classes=100)
        else:
            print(f"ResNet50 not available for {args.dataset}, using regular CNN")
            if args.dataset == 'mnist':
                global_model = CNNMnist(args=args)
            elif args.dataset == 'fmnist':
                global_model = CNNFashion_Mnist(args=args)
        ...
        if args.dataset == 'cifar':
            global_model = WideResNetCifar(depth=28, width=10, num_classes=10)
        elif args.dataset == 'cifar100':
            global_model = WideResNetCifar(depth=28, width=10, num_classes=100)
        else:
            print(f"Wide ResNet not available for {args.dataset}, using regular CNN")
            if args.dataset == 'mnist':
                global_model = CNNMnist(args=args)
            elif args.dataset == 'fmnist':
                global_model = CNNFashion_Mnist(args=args)
