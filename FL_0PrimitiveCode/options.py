...
def args_parser():
    ...
    parser.add_argument('--local_bs', type=int, default=10,
                        help="local batch size: B") # 客户端本地训练的批量大小
    ...
    # model arguments
    # 定义模型相关的参数
    parser.add_argument('--model', type=str, default='mlp', help='model name') # 使用的模型名称 (例如 'mlp', 'cnn')
    ...
    parser.add_argument('--dataset', type=str, default='mnist', help="name \
                        of dataset: mnist, fmnist, cifar, cifar100") # 使用的数据集名称
    parser.add_argument('--num_classes', type=int, default=10, help="number \
                        of classes: 10 for mnist/fmnist/cifar, 100 for cifar100") # 数据集的类别数量
    parser.add_argument('--gpu', default=None, help="To use cuda, set \
                        to a specific GPU ID. Default set to use CPU.") # 指定使用的 GPU ID，默认为 CPU
    parser.add_argument('--optimizer', type=str, default='sgd', help="type \
                        of optimizer") # 优化器类型 (例如 'sgd', 'adam')
    ...
    if args.dataset in ['mnist', 'fmnist']:
        args.num_channels = 1  # 灰度图像
        args.num_classes = 10  # 10个类别
    elif args.dataset == 'cifar':
        args.num_channels = 3  # RGB彩色图像
        args.num_classes = 10  # 10个类别
    elif args.dataset == 'cifar100':
        args.num_channels = 3  # RGB彩色图像
        args.num_classes = 100  # 100个类别
    else:
        print(f"Warning: Unknown dataset '{args.dataset}'. Please manually set num_channels and num_classes.")
